...
def detect_faces(images, app):
    ...
    return np.array(all_embeddings), embedding_to_image, images_with_faces, np.array(embedding_quality)
...
def organize_photos(models_folder, output_folder, distance_threshold=0.35, merge_threshold=0.75):
    """Main function to organize photos by person with high precision."""
    
    print("=" * 70)
    print("FACIAL RECOGNITION PHOTO ORGANIZER - HIGH PRECISION")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    embeddings, embedding_to_image, images_with_faces, quality_scores = detect_faces(images, app)
    ...
    labels, embeddings_normalized = cluster_faces_precise(embeddings, distance_threshold)
    ...
    potential_merges = find_similar_clusters(centroids, labels, embeddings_normalized, merge_threshold)
    
    if potential_merges:
        print(f"      Found {len(potential_merges)} potential merge(s) (similarity >= {merge_threshold}):")
        for merge in potential_merges[:10]:  # Show top 10
            print(f"        Person_{merge['cluster_i']:02d} <-> Person_{merge['cluster_j']:02d}: "
...
if __name__ == "__main__":
    ...
    MERGE_SUGGESTION_THRESHOLD = 0.70
    ...
    organize_photos(MODELS_FOLDER, OUTPUT_FOLDER, DISTANCE_THRESHOLD, MERGE_SUGGESTION_THRESHOLD)
